                            ...
                            ForEach(filteredResults.indices, id: \.self) { index in
                                if selectedType == .brand,
                                   let brand = filteredResults[index] as? SearchBrand {
                                    ...
                                    Button(action:{
                                        viewModel.currentBrandId = product.brandId
                                        previousState = currentState
                                        currentState = .brand
                                    }) {
...
//    @Binding var hasSearched: Bool
//    
//    // MainPage에서 바인딩으로 받아오는 새로고침 트리거 (옵셔널, 필요시만)
...
//                    .foregroundColor(Color.nickBoxStroke)
//                    .focused($isSearchFocusedInternal)
//                    .onChange(of: isSearchFocusedInternal) { oldFocus, newFocus in
//                        isFocused = newFocus
//                    }
//            }
//            
...
//                            if selectedType == .brand,
//                               let brand = filteredResults[index] as? SearchBrand {
//                                // refreshTrigger가 있으면 넘기고, 아니면 기존대로
//                                if let refreshTrigger = refreshTrigger {
//                                    NavigationLink(destination: BrandPage(brandId: brand.brandId, refreshTrigger: refreshTrigger)) {
//                                        SearchBrandCardView(brand: brand)
//                                    }
//                                } else {
...
//                            } else if selectedType == .product,
//                                      let product = filteredResults[index] as? SearchProduct {
//                                // 상품에서 브랜드 페이지로 이동 시에도 동일하게 처리
//                                if let refreshTrigger = refreshTrigger {
//                                    NavigationLink(destination: BrandPage(brandId: product.brandId, refreshTrigger: refreshTrigger)) {
//                                        SearchProductCardView(product: product)
//                                    }
//                                } else {
