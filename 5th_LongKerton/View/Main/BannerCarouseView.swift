...
struct BannerCarouselView: View {
    ...
    let userName = Text("\(UserSessionManager.shared.userData?.nickname ?? "장마엔 로그인")").font(.custom("Pretendard-SemiBold.ttf",size: 13))
        .foregroundColor(Color("BannerHeadTextColor"))
    
    let hello = Text("님 반가워요").font(.custom("Pretendard-Light.ttf",size: 13))
        .foregroundColor(Color("BannerHeadTextColor"))
